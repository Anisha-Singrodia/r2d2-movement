...
# toy = scanner.find_toy(toy_name = 'D2-7FE5')
...
def drive_with_commands(droid: SpheroEduAPI, roll_commands: List[Tuple[int, int, float]]):
    for command in roll_commands:
        droid.roll(command[0], command[1], command[2])



def sort_lambda(roll_commands: List[Tuple[int, int, float]]):
   return sorted(roll_commands, key=lambda x: (x[2], x[1]), reverse=False)


# 2. Functions (Drive Regular Polygon) [5 points]
def drive_polygon(droid: SpheroEduAPI, n: int, speed: int = 100, duration: float = 1.5):
    commands = [(180 - (((n-2)*180)//n), speed, duration) for angle in range(n)]
    intial_angle = 0
    list_comm = []
    for command in commands:
        intial_angle += command[0]
        new_comm = (intial_angle%360, command[1], command[2])
        print(new_comm)
        list_comm.append(new_comm)

    drive_with_commands(droid, list_comm)
...
def set_lights(droid: SpheroEduAPI, color: str, which_light='both'):
    ...
    if color in color_names_to_rgb:
        return color_names_to_rgb(color)
    else:
        return hex_names_to_rgb(color)    

# Color Dictionary
hex_names_to_rgb = {
    'FF0000': Color(255, 0, 0),
    'FF9900': Color(255, 165, 0),
    'FFFF00': Color(255, 255, 0),
    '008000': Color(0, 128, 0),
    '0000FF': Color(0, 0, 255),
    '660099': Color(75, 0, 130),
    'FF99FF': Color(238, 130, 238),
    '993399': Color(128, 0, 128)
}
def hex2rgb(hex_code: str) -> Color:
    return hex_names_to_rgb[hex_code]
...
def main():
    ...
    with SpheroEduAPI(scanner.find_toy(toy_name = 'D2-7FE5')) as droid:
            # Square Trajectory
            roll_commands = [(0, 100, 1), (90, 100, 1), (180, 100, 1), (270, 100, 1)]
            # roll_commands = [(0, 10, 2), (90, 900, 1), (180, 1, 12), (270, 100, 1)]
            print(sort_lambda(roll_commands))
